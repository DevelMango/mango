...
#if defined(NDEBUG)
#error "mango cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "mango Signed Message:\n";
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if(nHeight > 0 && nHeight <= 200) {
		...
		if (mNodeCoins == 0) {
			...
			LogPrintf("[mango] Adjusting mango at height %d with %d masternodes (%d % locked Nodex) and %d Nodex supply at %ld\n", nHeight, nMasternodeCount, lockedCoinValue*100, nMoneySupply, GetTime());
			LogPrintf("[mango] Masternode: %d\n", masternodeMultiplier*100);
			LogPrintf("[mango] Staker: %d\n", (1 - masternodeMultiplier)*100);
