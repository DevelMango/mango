...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/mango.conf are parsed in qt/mango.cpp's main()
    ParseParameters(argc, argv);

    // Process help and version before taking care about datadir
    if (mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("mango Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n";
    ...
    try {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "mango:"))
                fCommandLine = true;
        ...
        if (fDaemon) {
            fprintf(stdout, "mango server starting\n");
