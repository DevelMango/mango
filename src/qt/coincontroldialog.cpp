...
// click on header
    ...
    ui->treeWidget->setColumnWidth(COLUMN_ADDRESS, 250); // 190
    ui->treeWidget->setColumnHidden(COLUMN_DARKSEND_ROUNDS, true);
    ui->treeWidget->setColumnWidth(COLUMN_DATE, 108);
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::mngo;
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        BOOST_FOREACH (const COutput& out, coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show mango address. In tree mode, address is not shown again for direct wallet address outputs
