...
*Setup instructions for a gitian build of mango using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the mango
...
Clone the git repositories for mango and gitian and then checkout the mango version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the mango-core repository
...
To build mango (for Linux, OSX, and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the mango-core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mango-core/.git/
...
./bin/gbuild --commit mango=${COMMIT} --url mango=${URL} ../mango/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mango-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mango-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/mango-build.assert
```
